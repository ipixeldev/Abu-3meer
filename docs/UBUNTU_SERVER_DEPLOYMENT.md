...
YouTube membership does not require Google OAuth credentials on the server.
Configure only the public creator channel ID for latest-video discovery and the
CSV snapshot age. The app sends the user's manually entered channel profile
link when they tap Check membership; the API extracts the stable channel ID
and checks it against the current, unexpired full export. Follow
[YOUTUBE_MEMBERSHIP_CSV_CLAIMS.md](YOUTUBE_MEMBERSHIP_CSV_CLAIMS.md) for the
profile-link and complete-export workflow and its ownership limitation.
